-- 初始化脚本 - MySQL 5.7兼容版本
-- 注意：不需要CREATE DATABASE，因为环境变量会创建

USE jacoco;

-- 创建 diff_coverage_report 表
CREATE TABLE IF NOT EXISTS `diff_coverage_report` (
  ...
  `line_coverage` decimal(5,2) NOT NULL DEFAULT '-1.00' COMMENT '行覆盖率',
  `branch_coverage` decimal(5,2) NOT NULL DEFAULT '-1.00' COMMENT '分支覆盖率',
  ...
  KEY `id` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='增量代码覆盖率';

-- 创建 diff_deploy_info 表
CREATE TABLE IF NOT EXISTS `diff_deploy_info` (
  ...
  PRIMARY KEY (`job_record_uuid`),
  KEY `id` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='服务部署地址';
