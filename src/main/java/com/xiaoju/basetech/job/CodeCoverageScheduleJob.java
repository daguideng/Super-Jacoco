...
public class CodeCoverageScheduleJob {
    ...
    @Value("${schedule.code-clone.fixed-delay:10000}")
    private long codeCloneFixedDelay;

    @Value("${schedule.code-clone.initial-delay:10000}")
    private long codeCloneInitialDelay;

    @Value("${schedule.reset-status.fixed-delay:600000}")
    private long resetStatusFixedDelay;

    @Value("${schedule.reset-status.initial-delay:10000}")
    private long resetStatusInitialDelay;

    @Value("${schedule.env-cov.fixed-delay:300000}")
    private long envCovFixedDelay;

    @Value("${schedule.env-cov.initial-delay:300000}")
    private long envCovInitialDelay;

    @Value("${schedule.coverage-report-num:10}")
    private int coverageReportNum;
     ...
     */
    @Scheduled(fixedDelayString = "${schedule.code-clone.fixed-delay:10000}", initialDelayString = "${schedule.code-clone.initial-delay:10000}")
     ...
     */
    @Scheduled(fixedDelayString = "${schedule.reset-status.fixed-delay:600000}", initialDelayString = "${schedule.reset-status.initial-delay:10000}")
     ...
     */
    @Scheduled(fixedDelayString = "${schedule.env-cov.fixed-delay:300000}", initialDelayString = "${schedule.env-cov.initial-delay:300000}")
    public void calculateEnvCov() {
        List<CoverageReportEntity> resList = coverageReportDao.queryCoverByStatus(Constants.JobStatus.SUCCESS.val(),
                Constants.CoverageFrom.ENV.val(), coverageReportNum);
